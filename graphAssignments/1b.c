...
graph markEdge(graph g, int start, int end, int weight) {
    if(!checkBounds(g, start, end))
        return g;

    g.matrix[start][end] = g.matrix[end][start] = weight;
...
int getWeightEdge(graph g, int start, int end) {
    return checkBounds(g, start, end) ? g.matrix[start][end] : -1;
}
...
void traverseGraph(graph g, int start, int increment, bool visited[]) {
    ...
    printf("Vertex %d visited, its weight from first node is %d\n", start + increment, getWeightEdge(g, start, 0));
    visited[start] = true;
